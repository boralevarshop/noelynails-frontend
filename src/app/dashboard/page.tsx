...
export default function Dashboard() {
  ...
  // Cache de Tema para evitar "flash" branco
  ...
  useEffect(() => {
    ...
    // Carrega do cache visual imediatamente
  ...
  const fetchDados = async (tenantId: string) => {
    try {
      ...
      localStorage.setItem('tenant_cache', JSON.stringify(dadosTenant)); // Atualiza cache
  ...
  if (!usuario) return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="animate-pulse flex flex-col items-center"><div className="h-12 w-12 bg-gray-300 rounded-full mb-4"></div><div className="h-4 w-32 bg-gray-300 rounded"></div></div></div>;
  ...
  if (tenant) {
      if (tenant.statusAssinatura === 'TRIAL') {
          ...
          planoLabel = <span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full font-bold border border-green-300 shadow-sm">💎 Teste: {dias} dias</span>;
  ...
  return (
    <div className="min-h-screen pb-20 md:pb-0 transition-colors duration-500" style={{ backgroundColor: corFundo }}>
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div className="shadow rounded-lg p-5 relative" style={{ backgroundColor: corTerciaria }}>
                    <div className="flex justify-between items-start">
                        ...
                        <button onClick={() => setMostrarValores(!mostrarValores)} className="text-gray-400 hover:opacity-70 transition" style={{ color: corPrincipal }}>{mostrarValores ? <IconEyeOpen /> : <IconEyeClosed />}</button>
                    </div>
                            ...
                            for (let i = 0; i < 5; i++) {
                                ...
                                dias.push(
                                    <div key={i} className="p-4 rounded-lg shadow border" style={{ backgroundColor: corTerciaria, borderColor: '#E5E7EB' }}>
                                        <h3 className="font-bold text-gray-700 capitalize mb-2 border-b pb-2">{i === 0 ? 'Hoje' : i === 1 ? 'Amanhã' : nomeDia} <span className="text-xs text-gray-400 font-normal">({dataString.slice(0,5)})</span></h3>
        ...
        {isDono && (
            <div className="mt-10 rounded-lg shadow-lg p-4 flex flex-col md:flex-row items-center justify-between text-white lg:col-span-3" style={{ backgroundColor: corPrincipal }}>
                <div className="mb-3 md:mb-0 text-center md:text-left"><h3 className="font-bold text-lg" style={{ color: corTexto }}>🚀 Divulgue seu Espaço!</h3><p className="text-white/80 text-sm" style={{ color: corTexto }}>Envie este link para seus {tema.labels.cliente.toLowerCase()}s agendarem sozinhos.</p></div>
                <div className="flex items-center gap-2 bg-white/10 p-2 rounded-md border border-white/20 w-full md:w-auto justify-between md:justify-start"><code className="text-xs md:text-sm font-mono truncate max-w-[200px] md:max-w-none" style={{ color: corTexto }}>{linkPublico}</code><button onClick={() => navigator.clipboard.writeText(`https://${linkPublico}`).then(() => alert('Link copiado!'))} className="bg-white px-3 py-1 rounded text-xs font-bold hover:bg-gray-100 transition shrink-0" style={{ color: corPrincipal }}>Copiar</button></div>
            </div>
        )}
      </div>
