...
export default function Dashboard() {
  ...
  const [novoAgendamento, setNovoAgendamento] = useState({ nomeCliente: '', telefoneCliente: '', serviceId: '', professionalId: '' });
  ...
  useEffect(() => {
    ...
    } else if (modalAberto && usuario && filtroId && filtroId !== 'todos') {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/appointments`, {
        ...
        body: JSON.stringify({ ...novoAgendamento, tenantId: usuario.tenant.id, dataHora: dataHoraCombinada.toISOString() })
      });
      if (res.ok) {
          ...
          {agendamentosDoDia.length === 0 ? <p className="text-xs text-gray-400 italic text-center py-4">Livre</p> : (
            <ul className="space-y-2">
              {agendamentosDoDia.map((ag: any) => (
                <li key={ag.id} className="text-sm p-2 rounded border-l-4 bg-gray-50" style={{ borderLeftColor: tenant?.corPrimaria }}>
