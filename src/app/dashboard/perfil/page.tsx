...
export default function PerfilPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    instagram: '',
    avatarUrl: '',
    aparecerNoSite: true, // NOVO: Controle do Profissional
  ...
  const [formTenant, setFormTenant] = useState({
    ...
    agendamentoOnline: true // NOVO: Controle do Dono
  });
            ...
            <div className="p-6">
                ...
                {abaAtiva === 'dados' && (
                    <form onSubmit={handleSaveProfile} className="space-y-5 max-w-lg">
                        ...
                        {/* --- CONTROLE DE VISIBILIDADE (NOVO) --- */}
                        <div className="flex items-center gap-3 bg-indigo-50 p-4 rounded-lg border border-indigo-100">
                            <input 
                                type="checkbox" 
                                id="aparecerNoSite"
                                className="w-5 h-5 text-indigo-600 rounded cursor-pointer"
                                checked={formData.aparecerNoSite}
                                onChange={e => setFormData({...formData, aparecerNoSite: e.target.checked})}
                            />
                            <label htmlFor="aparecerNoSite" className="text-sm text-gray-700 font-medium cursor-pointer">
                                Permitir que clientes agendem comigo pelo site
                            </label>
                        </div>
                        {/* --------------------------------------- */}
                    ...
                    <form onSubmit={handleSaveTenant} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-4">
                                ...
                                {/* --- CONTROLE DE AGENDAMENTO ONLINE (NOVO) --- */}
                                <div className="flex items-center gap-3 bg-green-50 p-4 rounded-lg border border-green-100">
                                    <input 
                                        type="checkbox" 
                                        id="agendamentoOnline"
                                        className="w-5 h-5 text-green-600 rounded cursor-pointer"
                                        checked={formTenant.agendamentoOnline}
                                        onChange={e => setFormTenant({...formTenant, agendamentoOnline: e.target.checked})}
                                    />
                                    <div>
                                        <label htmlFor="agendamentoOnline" className="text-sm font-bold text-gray-800 cursor-pointer block">Site de Agendamento Ativo</label>
                                        <p className="text-xs text-gray-500">Se desmarcar, seu link público mostrará "Fechado temporariamente".</p>
                                    </div>
                                </div>
                                {/* ------------------------------------------- */}
